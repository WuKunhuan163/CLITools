...
class ToolEngine:
    ...
    def _(self, key, default, **kwargs):
        ...
        text = get_translation(str(self.project_root / "proj"), key, default)
    ...
    def install(self):
        ...
        if not self.tool_dir.exists():
            tm.add_stage(TuringStage(
                name=self._("label_fetching_source", "source"),
        ...
        # 6. Setup
        tm.add_stage(TuringStage(
            name=self._("setup_for_tool", "setup for {name} tool", name=self.tool_name),
            action=self.run_setup,
            fail_status=self._("setup_failed_label", "Setup failed"),
            success_status=self._("python_install_success_status", "Successfully ran"),
            success_color="WHITE"
        ))

        if tm.run(ephemeral=True):
            print(f"{self.BOLD}{self.GREEN}{self._('python_install_success_status', 'Successfully installed')}{self.RESET} {self.tool_name}")
            return True
        return False
        ...
        try:
            with open(tool_json_path, 'r') as f:
                ...
                for dep in deps:
                    ...
                    if not sub_engine.install(): return False
            return True
        except: return False
    ...
    def handle_pip_deps(self):
        ...
        req_path = self.tool_dir / "core" / "requirements.txt"
        ...
        utils_path = python_tool_dir / "core" / "utils.py"
        try:
...
if not py_tool.exists(): print("Error: PYTHON tool missing."); sys.exit(1)
