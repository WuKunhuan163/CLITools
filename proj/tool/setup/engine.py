...
class ToolEngine:
    ...
    def install(self, is_dependency=False):
        ...
        # 6. Setup
        tm.add_stage(TuringStage(
            name=self._("label_the_tool", "the {name} tool", name=self.tool_name),
            ...
            fail_status=self._("label_failed", "Failed"),
            success_name=self._("label_setup_success_name", "setup {name} tool", name=self.tool_name),
            fail_name=self._("label_setup_fail_name", "to setup the {name} tool", name=self.tool_name)
        ))
        ...
        if tm.run(ephemeral=True):
            ...
            # Rephrase failure message as requested
            # Failed (红色加粗) to setup the USERINPUT tool
            print(f"\r\033[K{self.BOLD}{self.RED}{self._('label_failed', 'Failed')}{self.RESET} {self._('label_setup_fail_name', 'to setup the {name} tool', name=self.tool_name)}")
            return False
