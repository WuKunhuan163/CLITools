...
class MinerUWrapper:
        ...
        try:
            ...
            if output_file:
                ...
                if async_mode and target_file:
                    print(f"Async mode enabled, calling add_async_placeholders", file=sys.stderr)
                    ...
                    self.add_async_placeholders(target_file, output_file, modified_pdf_path)
                else:
    ...
    def move_to_data_directory(self, source_file: str, pdf_path: str, call_api: bool = False, call_api_force: bool = False, page_range: Optional[str] = None) -> str:
        """Move output file to EXTRACT_PDF_DATA directory and create same-name file in PDF directory."""
        # Create data directory structure
        data_dir = Path(__file__).parent.parent / "EXTRACT_PDF_DATA"
        markdown_dir = data_dir / "markdown"
        markdown_dir.mkdir(parents=True, exist_ok=True)
        
        # Find next available filename for EXTRACT_PDF_DATA
        counter = 0
        while True:
        ...
        # Copy file to EXTRACT_PDF_DATA location
        shutil.copy2(source_file, target_file)
        
        # Copy images from MinerU temp directory to EXTRACT_PDF_DATA/images
        ...
        # Read the content from the EXTRACT_PDF_DATA file
        with open(target_file, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Update image paths to reference EXTRACT_PDF_DATA
    ...
    def update_image_paths_for_paper_directory(self, content: str, data_dir: str) -> str:
        """Update image paths in markdown content to reference EXTRACT_PDF_DATA directory."""
        import re
...
"""
            ...
            if pdf_path:
                try:
                    ...
                    if same_name_md_file.exists():
                        ...
                        # Copy images from EXTRACT_PDF_DATA to PDF directory images folder
                        data_dir = Path(__file__).parent.parent / "EXTRACT_PDF_DATA"
    ...
    def create_postprocess_status_json(self, pdf_path: str, blocks_to_process: list, image_path_to_type: dict):
        ...
        try:
            ...
            print(f"Error: Failed to create status file: {e}", file=sys.stderr)
            return None
        ...
        try:
            ...
            if status_file.exists():
                try:
                    ...
                    print(f"Warning:  Failed to read existing status file: {e}")
            ...
            print(f"Error: Failed to regenerate status file: {e}", file=sys.stderr)
            return None
    ...
    def add_hash_ids_to_status(self, status_file: str):
        ...
        try:
            ...
            print(f"Error: Failed to add hash ID: {e}", file=sys.stderr)
            return False
    ...
    def get_items_by_hash_ids(self, status_file: str, hash_ids: list) -> list:
        ...
        try:
            ...
            print(f"Error: Failed to get items by hash IDs: {e}", file=sys.stderr)
            return []
    ...
    def update_item_processing_status(self, status_file: str, hash_id: str, processed: bool = True):
        ...
        try:
            ...
            if updated:
                ...
                print(f"Warning:  ID not found: {hash_id}")
                return False
    ...
    def remove_placeholders_by_hash_ids(self, markdown_file: str, hash_ids: list):
        ...
        try:
            ...
            print(f"Error: Failed to remove placeholders: {e}", file=sys.stderr)
            return False
    ...
    def process_items_by_hash_ids(self, pdf_path: str, hash_ids: list, processing_type: str = 'all', custom_prompt: str = None):
        ...
        try:
            ...
            self.clean_existing_templates(str(markdown_file))
            ...
            for item in selected_items:
                ...
                if processed_content:
                    ...
                    if self.is_valid_processed_content(processed_content, item_type):
                        ...
                        print(f"Warning:  Invalid content, skipping replacement: {item_id}")
    ...
    def find_image_file(self, pdf_directory: Path, image_filename: str) -> Optional[str]:
        ...
        possible_locations = [
            ...
            Path(__file__).parent.parent / "EXTRACT_PDF_DATA" / "images" / image_filename
        ]
    ...
    def is_valid_processed_content(self, content: str, item_type: str) -> bool:
    ...
    def clean_existing_templates(self, markdown_file: str) -> bool:
    ...
    def load_hash_mapping(self) -> dict:
        ...
        try:
            ...
            print(f"Warning:  Failed to load hash mapping: {e}", file=sys.stderr)
            return {}
    ...
    def save_hash_mapping(self, mappings: dict):
        ...
        try:
            ...
            print(f"Error: Failed to save hash mapping: {e}", file=sys.stderr)
            return False
    ...
    def update_hash_mapping_from_items(self, items: list):
        ...
        try:
            ...
            print(f"Warning:  Failed to update hash mapping: {e}", file=sys.stderr)
    ...
    def update_image_cache_with_types(self, pdf_path: str):
        ...
        try:
            ...
            status_file = pdf_directory / f"{pdf_stem}_postprocess.json"
            if not status_file.exists():
                print(f"Warning:  Post-processing status file does not exist: {status_file}")
                return False
            ...
            print(f"Error: Failed to update image cache types: {e}", file=sys.stderr)
            return False
...
def extract_and_analyze_pdf_with_mineru(
    ...
    try:
        ...
        print(f"Warning:  MinerU failed, falling back to original PDF extractor", file=sys.stderr)
        print(f"MinerU: Unexpected error: {e}", file=sys.stderr)
