...
def clean_data_directory():
    """Clean the EXTRACT_PDF_DATA directory."""
...
def check_and_show_post_processing_status(result_path: str):
    ...
    try:
        ...
        print(f"Created at: {status_data.get('created_at', 'Unknown')}")
        ...
        if commands:
            ...
            print(f"- Process all: `{commands.get('process_all', 'N/A')}`")
...
def list_processing_items(pdf_path: str, item_type: str = 'all', show_processed: bool = False):
    ...
    try:
        ...
        for i, item in enumerate(filtered_items, 1):
            ...
            processed_info = f" (processing time: {processed_at})" if processed_at else ""
            
            print(f"{i:2d}. {status_icon} [{item['type'].upper()}] ID: {item['id'][:16]}...")
            print(f"     Processor: {item['processor']} | Page: {item['page']} | Status: {'processed' if item.get('processed', False) else 'not processed'}{processed_info}")
