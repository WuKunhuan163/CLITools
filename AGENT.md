...
|----------|------|---------|
| **Document** | Enhance README.md, AGENT.md for discoverability | After structural changes |
...
- Ensure AGENT.md and README.md exist and are comprehensive
...
- Ensure AGENT.md explains internals, gotchas, and integration patterns
...
- Browse `AGENT.md` of each tool — is it comprehensive or bare-minimum?
...
- **Waypoints**: If you discover a non-obvious connection between modules, add a note in the relevant AGENT.md (e.g., "This module is used by tool/X — see its AGENT.md for integration details")
...
- **Exploration circuits**: If you find a useful sequence of commands for a common task, add it to the `recipes` skill or the relevant tool's AGENT.md
- **Doc symmetry check**: Every tool should have both README.md and AGENT.md. If one is missing or bare-minimum, create/expand it during Curate or Document phases
...
- **Fix at source**: When a tool (including ecosystem tools like TOOL, BRAIN, SKILLS) errors, read its source and AGENT.md, fix the bug directly, and retry. If unfixable, search for alternatives. Ask the user only as last resort.
...
See `interface/AGENT.md` for the full module map. Direct `logic.*` imports are only for code inside `logic/` itself.
...
- **`dev`**: Active development branch. Does NOT contain `logic/_/dev/` (gitignored).
- **`tool`**: **Remote-only** staging branch. Contains `logic/_/dev/archived/` (archived tool code) and binary resources pushed via `--dev push-resource`. The `tool` branch is never kept locally — it is fetched on demand and cleaned up after use.
- **`main`**: Production-ready framework only (no `tool/`, `logic/_/dev/`, `data/`, `bin/`, `tmp/`).
...
|---|---|---|---|
| Tool source code | `tool/<NAME>/` | Yes | `TOOL --dev sync` archives to `logic/_/dev/archived/` |
| Binary resources | `logic/_/dev/resource/<NAME>/` | No (gitignored) | `TOOL --dev push-resource <NAME>` |
| Archived tool snapshots | `logic/_/dev/archived/<NAME>/` | No (gitignored) | Created during `TOOL --dev sync` alignment |
...
4. Fallback: `origin/tool` → `logic/_/dev/archived/<NAME>/` (copied to `tool/<NAME>/`)
...
- **`TOOL --dev archive <name>`**: Archive a tool to `logic/_/dev/archived/`.
...
**IMPORTANT for skills discoverability**: If a user asks you to locate a skill and you cannot find it, you should create a new skill and adjust relevant README.md / AGENT.md files to improve discoverability for future agents.
...
Tools import shared framework utilities from `interface.*`. See `interface/AGENT.md` for the full module map. Key modules:
...
cat tool/TOOLNAME/AGENT.md
cat tool/TOOLNAME/README.md
```

Every tool has a `AGENT.md` with architecture details and a `README.md` with usage docs. Read them first.
...
3. Check `AGENT.md` and README of involved tools.
...
1. Update the nearest `AGENT.md` and `README.md` to reference the new work.
2. Trace the documentation hierarchy upward — if a tool-level doc change is needed, also check whether `logic/AGENT.md` or even the root `AGENT.md` needs a mention.
3. For new skills: ensure `skills/core/skills-index/SKILL.md` includes it.
4. For new infrastructure: ensure `interface/AGENT.md` lists the new module.
5. For new tools: ensure `AGENT.md` Section 10 references the tool.

The test: imagine an agent that has never seen this project before and only reads `AGENT.md`. Can it find your work? If not, add breadcrumbs.
