...
class TestGDSStateManager(unittest.TestCase):
    """Test state manager operations using a temporary directory."""

    def setUp(self):
        self.tmpdir = Path(tempfile.mkdtemp())
        tool_dir = self.tmpdir / "tool" / "GOOGLE.GDS" / "data"
        tool_dir.mkdir(parents=True, exist_ok=True)

        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "gcs_state",
            str(project_root / "tool" / "GOOGLE.GDS" / "logic" / "state.py")
        )
        state_mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(state_mod)
        self.GDSStateManager = state_mod.GDSStateManager
        self.mgr = self.GDSStateManager(self.tmpdir)
    ...
    def test_06_persistence(self):
        ...
        mgr2 = self.GDSStateManager(self.tmpdir)
