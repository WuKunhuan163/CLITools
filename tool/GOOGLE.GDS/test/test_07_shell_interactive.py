...
class TestShellInteractive(unittest.TestCase):
    """Test GDS --shell interactive mode entry and exit."""
    ...
    def test_01_shell_entry_and_exit(self):
        ...
        self.assertIn("GDS Interactive Shell", output)
    ...
    def test_02_shell_help_command(self):
        ...
        output = self._strip_ansi(stdout)
        self.assertIn("GDS Shell Commands", output)
    ...
    def test_03_shell_pwd_command(self):
        """Typing 'pwd' in shell should invoke GDS pwd and output a path."""
        proc = subprocess.Popen(
            [sys.executable, str(self.gcs_bin), "--no-warning", "--shell"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
        ...
        output = self._strip_ansi(stdout)
        self.assertIn("GDS Interactive Shell", output)
        # pwd output may appear inline with the prompt (e.g., "GDS:~$ ~")
