#!/usr/bin/env python3
"""GDS venv command: manage remote Python virtual environments."""
import os
import sys
import json
import base64
...
def _run_remote_cmd(tool, state_mgr, load_logic, utils, command):
    ...
    cdp_enabled = os.environ.get("GDS_CDP_ENABLED") == "1"
    ...
    def gui_action(stage=None):
        ...
        logic_script = str(tool.project_root / "tool" / "GOOGLE.GDS" / "logic" / "executor.py")
    ...
    pm = ProgressTuringMachine(project_root=tool.project_root, tool_name="GOOGLE.GDS", log_dir=tool.get_log_dir())
...
def _venv_create(tool, state_mgr, load_logic, utils, env_names):
    ...
    env_folder_id = config.get("env_folder_id")
    if not env_folder_id:
        print(f"{BOLD}{RED}Error{RESET}: env folder not configured. Run 'GDS --setup-tutorial'.", file=sys.stderr)
        return 1
...
def _venv_list(tool, state_mgr, load_logic, utils):
    ...
    env_folder_id = config.get("env_folder_id")
    if not env_folder_id:
        print("Env folder not configured. Run 'GDS --setup-tutorial'.", file=sys.stderr)
        return 1
    ...
    if not env_names:
        ...
        print("Create one with: GDS venv --create <name>")
        return 0
...
def _venv_current(tool, state_mgr, load_logic, utils):
    ...
    if current:
        ...
        print("Activate one with: GDS venv --activate <name>")
    return 0
...
Usage:
  GDS venv --create <name> [name2 ...]   Create virtual environment(s)
  GDS venv --delete <name> [name2 ...]   Delete virtual environment(s)
  GDS venv --activate <name>             Activate virtual environment
  GDS venv --deactivate                  Deactivate current environment
  GDS venv --list                        List all virtual environments
  GDS venv --current                     Show currently active environment
  GDS venv --protect <name> [...]        Protect environment from deletion
  GDS venv --unprotect <name> [...]      Remove protection
...
Examples:
  GDS venv --create myproject            Create 'myproject' environment
  GDS venv --activate myproject          Activate 'myproject'
  GDS venv --list                        List all environments
  GDS venv --current                     Check current environment
  GDS venv --deactivate                  Return to base environment
  GDS venv --delete myproject            Delete 'myproject'
  GDS venv --protect production          Protect from accidental deletion

Related:
  GDS python --help                      Python execution
  GDS pip --help                         Package management""")
