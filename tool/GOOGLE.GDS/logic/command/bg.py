#!/usr/bin/env python3
"""GDS bg command: run remote commands in background with status tracking."""
import os
import sys
import json
import time
...
"""
    ...
    if ok:
        ...
        if exit_code == 0:
            ...
            print()
            print("Track with:")
            print(f"  GDS bg --status {bg_pid}    Check status")
            print(f"  GDS bg --log {bg_pid}       View output log")
            print(f"  GDS bg --result {bg_pid}    View result")
            print(f"  GDS bg --cleanup {bg_pid}   Clean up files")
            return 0
        if stdout:
            print(stdout, end="")
...
def _bg_result(tool, state_mgr, load_logic, utils, task_id):
    ...
    def format_result(content):
        ...
        if content == "NOT_FOUND":
            ...
            print(f"Use 'GDS bg --status {task_id}' to check if it's still running.")
            return
...
Usage:
  GDS bg <command>                       Run command in background
  GDS bg --status [task_id]              Show task status (all or specific)
  GDS bg --log <task_id>                 View task output log
  GDS bg --result <task_id>              View task result (stdout/stderr)
...
Workflow:
  1. Submit:  GDS bg "pip install torch"
  2. Check:   GDS bg --status <task_id>
  3. View:    GDS bg --log <task_id>
  4. Result:  GDS bg --result <task_id>
  5. Cleanup: GDS bg --cleanup <task_id>

Examples:
  GDS bg "pip install numpy pandas"      Install packages in background
  GDS bg "git clone https://..."         Clone repo in background
  GDS bg --status                        List all background tasks
  GDS bg --cleanup                       Clean up all completed tasks""")
