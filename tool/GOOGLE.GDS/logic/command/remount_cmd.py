#!/usr/bin/env python3
"""GDS --remount command: generate and execute Colab remount script via GUI."""
import os
import sys
import json
import time
...
_DEBUG_LOG = Path(__file__).resolve().parent.parent.parent.parent.parent / "tmp" / "remount_debug.log"
...
def _execute_cdp(tool, remount_mod, script, metadata, no_feedback=False):
    ...
    def _apply_overlays(session):
        ...
        if not ov or not session:
            return
        try:
            ov.inject_badge(session, text="GDS [remount]", color="#0d904f")
    ...
    pm = ProgressTuringMachine(
        project_root=tool.project_root, tool_name="GOOGLE.GDS", log_dir=tool.get_log_dir()
    )
