#!/usr/bin/env python3
"""GDS --raw / --no-capture command: run commands in Colab with real-time output.
...
def execute(tool, remote_command, state_mgr, load_logic, no_capture=False, no_feedback=False, **kwargs):
    ...
    finished_msg_nc = _t(tool, "remote_finished_no_capture",
                         "(exit code: $?). Press Finished in the GDS window.")
    if no_capture:
    ...
    if metadata.get("done_marker"):
        gui_args.extend(["--done-marker", f"GDS_DONE_{metadata['hash']}"])
    ...
    hint = _t(tool, "hint_drive_not_mounted",
              "If Google Drive is not mounted, run '{bold}GDS --remount{reset}' first.",
...
'''
    ...
    return f'''cat > /tmp/gcs_raw_{script_id}.sh << 'GDS_RAW_EOF'
{inner_script}GDS_RAW_EOF
...
'''
    ...
    return f'''cat > /tmp/gcs_nc_{script_id}.sh << 'GDS_NC_EOF'
{inner_script}GDS_NC_EOF
...
def _t(tool, key, default, **kwargs):
    try:
        ...
        logic_dir = str(tool.project_root / "tool" / "GOOGLE.GDS" / "logic")
...
def _run_userinput_feedback(tool):
    ...
    if interface_path.exists():
        try:
            ...
            return get_user_feedback(
                hint="GDS Raw Mode: Please provide feedback on the command output.",
                title="GDS Raw Feedback"
            )
        except Exception:
            pass
    ...
    if not userinput_bin.exists():
        return None
    res = subprocess.run(
        [str(userinput_bin), "--hint", "GDS Raw Mode: Please provide feedback on the command output."],
...
def _set_failure_reason(stage, res, tool=None):
    ...
    if status == "cancelled" or reason in ("interrupted", "signal", "stop"):
        ...
        stage.error_brief = _t(tool, "turing_drive_not_mounted",
                               "Google Drive not mounted. Run 'GDS --remount' first.") if tool else "Google Drive not mounted. Run 'GDS --remount' first."
