...
def generate_remount_script(project_root: Path):
    ...
    if not config_path.exists():
        return None, "GDS config not found. Please run 'GDS setup-tutorial'."
    ...
    if not root_folder_id or not env_folder_id:
        return None, "GDS folder IDs not found in config. Please run 'GDS setup-tutorial'."

    # 2. Load Service Account Key
    key_path = project_root / "data" / "google_cloud_console" / "console_key.json"
    if not key_path.exists():
        return None, "Service account key not found. Please run 'GDS setup-tutorial'."
    ...
    script_template = r'''# GDS Remount Script
import os
import json
...
def show_remount_gui(project_root: Path, script: str, metadata: dict, mcp_mode=False):
            ...
            if userinput_bin:
                subprocess.Popen(
                    [userinput_bin, "--enquiry",
                     "--hint", "GDS remount feedback: please describe the issue or provide instructions."],
    ...
    gui_title = _("gui_title_remount", "GDS Remount")
    if mcp_mode:
        gui_title += " [MCP]"
