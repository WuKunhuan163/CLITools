...
def resolve_path_via_api(project_root: Path, path: str, current_path: str = "~", current_folder_id: str = None, timeout: int = 60) -> Tuple[Optional[str], str]:
    ...
    if path in ("~", "~/"):
        ...
        if not fid:
            return None, "Root folder not configured. Run 'GDS --setup-tutorial'."
        return fid, "~"

    if path in ("@", "@/"):
        fid = config.get("env_folder_id")
        if not fid:
            return None, "Env folder not configured. Run 'GDS --setup-tutorial'."
        return fid, "@"
...
def _write_debug_log(project_root: Path, msg: str):
    try:
        log_dir = project_root / "tool" / "GOOGLE.GDS" / "data" / "log"
