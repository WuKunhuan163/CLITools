...
def _get_supported_types():
    try:
        from tool.GOOGLE.interface.main import DRIVE_MIME_TYPES
...
def run_mcp_create(file_type, folder_spec="~", filename=None, as_json=False):
    ...
    if not folder_id:
        if as_json:
            print(json.dumps({"status": "error", "message": f"Cannot resolve folder '{folder_spec}'. Run GDS --setup-tutorial first."}))
        else:
            print(f"{BOLD}{RED}Failed to resolve{RESET} folder '{folder_spec}'.")
            print(f"  Run {BOLD}GDS --setup-tutorial{RESET} first.")
        return 1

    if _is_cdp_available():
        from tool.GOOGLE.interface.main import create_drive_file
    ...
    if as_json:
        ...
        print(f"\n  Use {BOLD}GDS --mcp-create {file_type} --json{RESET} for structured workflow instructions.")
    return 2
...
def run_mcp_delete(file_id, as_json=False):
    ...
    if _is_cdp_available():
        from tool.GOOGLE.interface.main import delete_drive_file
    ...
    if as_json:
        ...
        print(f"  Start debug Chrome with {BOLD}GDS --mcp boot{RESET} first.")
    return 1
...
def run_mcp_upload(folder_spec="~", as_json=False):
    ...
    if as_json:
        ...
        print(f"\n  Use {BOLD}GDS --mcp-upload --json{RESET} for structured workflow instructions.")
    return 2
