...
def run_mcp_boot():
    """GDS --mcp boot: Launch debug Chrome and open a Colab tab."""
...
def run_mcp_shutdown():
    """GDS --mcp shutdown: Close the debug Chrome instance."""
...
def run_mcp_status():
    """GDS --mcp status: Show CDP readiness and config state."""
    cfg = _load_config()
    ...
    if not setup_done:
        print(f"\n  Run {BOLD}GDS --setup-tutorial{RESET} to configure Google Drive folders.")
    elif not cdp_ok:
        print(f"\n  Run {BOLD}GDS --mcp boot{RESET} to start Chrome CDP.")
    elif not tab:
        print(f"\n  Open any Colab notebook, or run {BOLD}GDS --mcp boot{RESET}.")
...
def run_mcp_setup_tutorial():
    """GDS --mcp setup-tutorial: Launch the GUI-based MCP setup tutorial.

    Prerequisites: GDS --setup-tutorial should be completed first (folders configured).
    ...
    if not (env_id or root_id):
        ...
        print(f"  Run {BOLD}GDS --setup-tutorial{RESET} first, then re-run this command.")
        return 1
