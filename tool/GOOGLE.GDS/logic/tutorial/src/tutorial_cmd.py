#!/usr/bin/env python3
"""GDS --setup-tutorial command: run the interactive setup wizard."""
import sys
...
def execute(tool, **kwargs):
    tutorial_path = Path(__file__).resolve().parent.parent / "setup_guide" / "main.py"
    ...
    if res.get("status") == "success":
        ...
        print(f"{get_color('BOLD')}{get_color('GREEN')}{success_label}{get_color('RESET')} {_('success_setup_tutorial', 'GDS setup tutorial.')}")
        return 0
    else:
