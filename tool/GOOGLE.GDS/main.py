...
def main():
    os.environ.pop("GDS_CDP_ENABLED", None)
    ...
    tool = ToolBase("GOOGLE.GDS")
    
    parser = argparse.ArgumentParser(description="Google Drive Remote Controller (GDS)", add_help=False)
    ...
    parser.add_argument("--setup-tutorial", action="store_true", help="Run the GDS setup tutorial")
    ...
    if _gui_match:
        ...
        sys.exit(handle_gui_remote_command("GOOGLE.GDS", project_root, _gui_cmd_map[_gui_match], remaining, tool.get_translation))

    # Early intercept --mcp: management subcommands or decorator
    # Management: GDS --mcp boot | shutdown | status | setup-tutorial
    # Decorator:  GDS <command> --mcp [--json]
    ...
    if "--mcp" in sys.argv:
        ...
        if next_arg_clean in _MCP_MANAGEMENT_CMDS:
            ...
            os.environ["GDS_CDP_ENABLED"] = "1"
            sys.argv = [a for a in sys.argv if a not in ("--mcp", "--json")]
        else:
            # MCP decorator: GDS <command> --mcp
            # With CDP: run the normal GDS flow (executor.py handles CDP auto-injection)
            ...
            if _cdp_ok:
                ...
                print(f"{get_color('BOLD')}{get_color('YELLOW')}Chrome CDP not available{get_color('RESET')}. Run {get_color('BOLD')}GDS --mcp boot{get_color('RESET')} first.")
                print(f"  If not yet configured, run {get_color('BOLD')}GDS --mcp setup-tutorial{get_color('RESET')}.\n")
                clean = [a for a in sys.argv[1:] if a not in ("--mcp", "--json", "--no-warning", "--python")]
                as_json_flag = "--json" in sys.argv
                as_python_flag = "--python" in sys.argv
                command = " ".join(clean)
                if not command:
                    print(f"{get_color('BOLD')}{get_color('RED')}Missing command{get_color('RESET')}. Usage: GDS --mcp <boot|shutdown|status|setup-tutorial> or GDS <command> --mcp")
                    sys.exit(1)
                mcp_exec = _load_mcp_module("mcp/execute")
                sys.exit(mcp_exec.run_mcp_execute(command, as_python=as_python_flag, as_json=as_json_flag))
    ...
    if "--mcp-create" in sys.argv:
        ...
        if rest:
            folder_spec = rest[0]
        if not file_type:
            print(f"{get_color('BOLD')}{get_color('RED')}Missing type{get_color('RESET')}. Usage: GDS --mcp-create TYPE [FOLDER] [--name NAME]")
            sys.exit(1)
    ...
    if "--mcp-delete" in sys.argv:
        ...
        if not file_id:
            print(f"{get_color('BOLD')}{get_color('RED')}Missing file ID{get_color('RESET')}. Usage: GDS --mcp-delete FILE_ID")
            sys.exit(1)
        mcp_cr = _load_mcp_logic("mcp/create")
    ...
    state_mgr = state_mod.GDSStateManager(tool.project_root)
...
def _handle_reconnection(tool, reconnection_args):
    ...
    from interface.config import get_color
    gcs_logic_dir = str(tool.project_root / "tool" / "GOOGLE.GDS" / "logic")
    ...
    if sub == "status":
        ...
        print(f"{BOLD}GDS API Reconnection Status{RESET}")
        ...
        if len(reconnection_args) < 3:
            print(f"{RED}Usage:{RESET} GDS --reconnection config <command_count> <duration_seconds>")
            return
        try:
        ...
        print(f"Usage: GDS --reconnection [status|config <count> <duration>|reset]")
