...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI image generation via Midjourney API", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}MIDJOURNEY{RESET} tool status:")
        print(f"  MCP package: mcp-midjourney")
        print(f"  Type: pip")
        print(f"  Capabilities: text-to-image, image-transform, upscale, variations, blend")
        return 0

    if args.command == "config":
        if not args.args or len(args.args) < 2:
            print(f"Usage: MIDJOURNEY config <key> <value>")
            return 1
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: text-to-image, image-transform, upscale, variations, blend")
    return 0
