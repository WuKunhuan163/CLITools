...
def main():
    ...
    parser = argparse.ArgumentParser(description="GitLab integration via GitLab MCP", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}GITLAB{RESET} tool status:")
        print(f"  MCP package: gitlab-mcp")
        print(f"  Type: npm")
        print(f"  Capabilities: repositories, issues, merge-requests, pipelines")
        return 0

    if args.command == "config":
        if not args.args or len(args.args) < 2:
            print(f"Usage: GITLAB config <key> <value>")
            return 1
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: repositories, issues, merge-requests, pipelines")
    return 0
