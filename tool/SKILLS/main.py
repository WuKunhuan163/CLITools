...
BRAIN_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "_" / "runtime" / "_" / "eco" / "experience"
...
def main():
    ...
    list_p.add_argument("--core", action="store_true", help="Show only project-specific (core) skills")
    ...
    if args.command == "list":
        if getattr(args, "core", False):
            skills = sorted(_collect_skills_from(PROJECT_SKILLS_DIR), key=lambda x: x[0])
        else:
            skills = get_skills()
        if not skills:
