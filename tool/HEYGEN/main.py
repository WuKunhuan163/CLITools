...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI avatar video generation via HeyGen API", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}HEYGEN{RESET} tool status:")
        print(f"  MCP package: heygen-mcp-server")
        print(f"  Type: npm")
        print(f"  Capabilities: avatar-management, voice-selection, video-generation")
        return 0

    if args.command == "config":
        if not args.args or len(args.args) < 2:
            print(f"Usage: HEYGEN config <key> <value>")
            return 1
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: avatar-management, voice-selection, video-generation")
    return 0
