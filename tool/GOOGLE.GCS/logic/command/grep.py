#!/usr/bin/env python3
"""GCS grep command: search for patterns in remote files."""
import re
import sys
...
Usage:
  GCS grep <pattern> <file> [file2 ...] [options]
  GCS grep <file>                         Display file with line numbers
...
Examples:
  GCS grep 'import' ~/tmp/script.py       Find lines with 'import'
  GCS grep -i 'error' ~/logs/app.log      Case-insensitive search
  GCS grep -c 'TODO' ~/src/main.py        Count TODO occurrences
  GCS grep script.py                      Display entire file""")
