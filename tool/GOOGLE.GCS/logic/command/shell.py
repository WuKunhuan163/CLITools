...
def enter_interactive(tool, state_mgr, load_logic, default_as_python=False):
    """Interactive REPL shell mode. Commands are dispatched as GCS subcommands."""
    BOLD = get_color("BOLD")
    ...
    print(f"{BOLD}GCS Interactive Shell{RESET}")
    print(f"Type commands as if prefixed with 'GCS'. Type {BOLD}exit{RESET} to quit, {BOLD}help{RESET} for usage.\n")
    ...
    while True:
        try:
            ...
            prompt = f"{BLUE}GCS{type_label}{RESET}:{GREEN}{display_path}{RESET}$ "
            try:
                user_input = input(prompt).strip()
            except (EOFError, KeyboardInterrupt):
                print(f"\n{BOLD}Exiting{RESET} GCS shell.")
                return
            ...
            if cmd in ("exit", "quit"):
                print(f"{BOLD}Exiting{RESET} GCS shell.")
                return
            ...
            tool_bin = os.environ.get("GCS_BIN_PATH", "GCS")
            ...
            try:
                ...
                print(f"{BOLD}{RED}Error{RESET}: GCS executable not found. Ensure GCS is installed.")
...
def manage(tool, shell_args, state_mgr, **kwargs):
    ...
    if action == "list":
        ...
        if not name_or_id:
            print("Usage: GCS --shell create <name>")
            return
        ...
        if not name_or_id:
            print("Usage: GCS --shell switch <id>")
            return
        ...
        if not name_or_id:
            print("Usage: GCS --shell install <shell_name>  (e.g. zsh, fish)")
            return
...
def _print_help():
    BOLD = get_color("BOLD")
    RESET = get_color("RESET")
    print(f"{BOLD}GCS Shell Commands:{RESET}")
    ...
    print(f"\nOther input is sent as a remote command (via GCS).")
