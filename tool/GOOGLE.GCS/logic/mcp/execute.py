"""MCP workflow for executing GCS commands via Colab browser automation.

Architecture:
    1. GCS command runs normally — opens a Tkinter GUI window (ButtonBarWindow)
    ...
    3. After output appears, agent sends `GCS --gui-submit` to close the GUI.
    4. Normal GCS flow resumes (result download, etc.).

The GCS GUI Queue ensures multiple --mcp sessions are serialized.
    ...
    steps = [
        {
            ...
            "description": "Run the GCS command normally (opens GUI window, copies script to clipboard)",
            "shell_command": f"GCS {command}",
        ...
        },
        {
            "step": 12,
            "action": "submit_gui",
            "description": "Close the GCS GUI window via external control",
            "shell_command": "GCS --gui-submit",
            "note": "This triggers the normal GCS flow to continue (result download, etc.).",
        },
    ]
...
def run_mcp_execute(command, as_python=False, as_json=False):
    """Execute a GCS command via MCP/CDP automation.
    ...
    print(f"  Flow:   GCS {command} -> type in Colab -> execute -> GCS --gui-submit")
    return 0
...
def _run_cdp_with_turing(command, as_python=False):
    ...
    pm = ProgressTuringMachine(project_root=str(_project_root), tool_name="GOOGLE.GCS")
    ...
    if success and output:
        ...
        for line in lines:
            if not line.startswith("GCS_DONE_"):
                print(f"  {line}")
