#!/usr/bin/env python3
"""
GCS comprehensive test suite adapted from GDS _UNITTEST/test_gds.py.

Contains the 100 most complex test scenarios from GDS, adapted for the GCS
...
class _GCSLocalBase(MCPTestCase):
    """Base for local-only GCS tests (no remote exec needed)."""
    ...
    def gcs_local(self, args, timeout=60):
        """Run a GCS local command (no MCP, no remote execution)."""
...
class _GCSTestBase(_GCSLocalBase):
    """Shared setup/teardown for remote GCS complex tests."""
    ...
    def gcs_ok(self, args, msg=None, **kw):
        """Run GCS command and assert success."""
        r = self.gcs(args, **kw)
        self.assertSuccess(r, msg)
        return r

    def gcs_fail(self, args, msg=None, **kw):
        """Run GCS command and assert failure."""
        r = self.gcs(args, **kw)
...
class TestFileOps(_GCSTestBase):
...
class TestNavigation(_GCSTestBase):
...
class TestRedirection(_GCSTestBase):
...
class TestBackground(_GCSTestBase):
    """bg from GDS test_24."""
...
class TestShell(_GCSLocalBase):
    ...
    def test_shell_04_type(self):
        """GCS --shell type shows shell type."""
