...
def main():
    ...
    parser.add_argument("--between", nargs=2, metavar=("START", "END"), help="Download photos/videos between two dates (YYYY-MM-DD YYYY-MM-DD)")
    ...
    def auth_action(stage=None):
        ...
        if creds.get("mode") == "session_reuse":
            ...
            try:
                ...
                if stage: stage.report_error("Session Reuse Failed", str(e) or "Could not restore saved session")
                return False
        ...
        try:
            api = PyiCloudService(apple_id, password)
            if api.requires_2fa:
                from interface.gui import TwoFactorAuthWindow
