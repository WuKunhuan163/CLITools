...
_APP_URL = "https://boardmix.com"
_URL_PATTERN = "boardmix.com"


def main():
    tool = ToolBase("BOARDMIX")

    parser = argparse.ArgumentParser(
        description="Boardmix collaborative whiteboard via CDMCP",
        epilog="MCP commands use --mcp- prefix: e.g., BOARDMIX --mcp-boot",
        add_help=False,
    )
    sub = parser.add_subparsers(dest="command", help="MCP subcommand")

    sub.add_parser("boot", help="Boot Boardmix CDMCP session")
...
def _cmd_boot(tool):
    ...
    result = sm.boot_tool_session(session_name="boardmix")
    ...
    if tab_info and tab_info.get("ws"):
        ...
        try:
            ...
            ov.inject_badge(cdp, text="BOARDMIX", color="#4A6CF7")
            ov.inject_focus(cdp, color="#4A6CF7")
            ov.inject_favicon(cdp, svg_color="#4A6CF7", letter="B")
            ov.inject_lock(cdp, base_opacity=0.08, flash_opacity=0.25, tool_name="BOARDMIX")
            cdp.close()
        except Exception:
            pass
        print(f"{BOLD}{GREEN}Successfully booted{RESET} Boardmix session.")
        print(f"  Tab: {tab_info.get('url', '?')[:60]}")
    else:
        print(f"{BOLD}{GREEN}Successfully booted{RESET} session (no Boardmix tab found).")
...
def _cmd_status(tool):
    ...
    tab = _find_tab()
    if not tab:
        print(f"{BOLD}{RED}No Boardmix tab{RESET} found. Run BOARDMIX --mcp-boot first.")
        return
...
def _cmd_state(tool):
    ...
    state["boardmix_tab"] = {"id": tab.get("id", ""), "url": tab.get("url", "")}
    try:
        s = CDPSession(tab["webSocketDebuggerUrl"], timeout=10)
...
def _cmd_session(tool):
    try:
        ...
        return
    session = smod.get_session("boardmix")
    if not session:
        print(f"{get_color('BOLD')}No active session{get_color('RESET')}. Run BOARDMIX --mcp-boot.")
        return
    BOLD = get_color("BOLD")
    RESET = get_color("RESET")
...
def _cmd_boards(tool):
    ...
    boards = s.evaluate("""
        (function(){
            var items = document.querySelectorAll('[class*="board"], [class*="card"], [class*="file"], a[href*="/board/"]');
            var out = [];
...
def _cmd_explore(tool):
    ...
    if img:
        out_path = Path("/Applications/AITerminalTools/tmp/boardmix_explore.png")
