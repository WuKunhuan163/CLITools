# PAYPAL Logic — Technical Reference

## Architecture

Standard CDMCP tool pattern:
- `chrome/api.py`: All CDP operations against `paypal.com`
- `mcp/main.py`: MCPToolConfig for MCP server registration

## chrome/api.py

Tab discovery: `find_paypal_tab()` searches for `paypal.com` via `logic.chrome.session.find_tab()`.

Data source: authenticated session + DOM.

Public API: `find_paypal_tab()`, `get_auth_state()`, `get_page_info()`, `get_account_info()`, `get_recent_activity()`.
