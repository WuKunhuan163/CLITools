...
_APP_URL = "https://miro.com"
_URL_PATTERN = "miro.com"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Miro collaborative whiteboard via CDMCP",
        epilog="MCP commands use --mcp- prefix: e.g., MIRO --mcp-boot",
        add_help=False,
    )
    sub = parser.add_subparsers(dest="command", help="MCP subcommand")

    sub.add_parser("boot", help="Boot Miro CDMCP session")
    ...
    sub.add_parser("boards", help="List boards on current page")
...
def _cmd_boot(tool):
    ...
    result = sm.boot_tool_session(session_name="miro")
    ...
    if tab_info and tab_info.get("ws"):
        ...
        try:
            ...
            ov.inject_badge(cdp, text="MIRO", color="#FFD02F")
            ov.inject_focus(cdp, color="#FFD02F")
            ov.inject_favicon(cdp, svg_color="#FFD02F", letter="M")
            ov.inject_lock(cdp, base_opacity=0.08, flash_opacity=0.25, tool_name="MIRO")
            cdp.close()
        except Exception:
            pass
        print(f"{BOLD}{GREEN}Successfully booted{RESET} Miro session.")
        print(f"  Tab: {tab_info.get('url', '?')[:60]}")
    else:
        print(f"{BOLD}{GREEN}Successfully booted{RESET} session (no Miro tab found).")
...
def _cmd_status(tool):
    ...
    tab = _find_tab()
    if not tab:
        print(f"{BOLD}{RED}No Miro tab{RESET} found. Run MIRO --mcp-boot first.")
        return
...
def _cmd_state(tool):
    ...
    state = {"cdp_available": False, "miro_tab": None, "page": {}}
    ...
    state["miro_tab"] = {"id": tab.get("id", ""), "url": tab.get("url", "")}
    try:
        s = CDPSession(tab["webSocketDebuggerUrl"], timeout=10)
...
def _cmd_session(tool):
    try:
        ...
        return
    session = smod.get_session("miro")
    if not session:
        print(f"{get_color('BOLD')}No active session{get_color('RESET')}. Run MIRO --mcp-boot.")
        return
    BOLD = get_color("BOLD")
    RESET = get_color("RESET")
...
def _cmd_boards(tool):
    ...
    tab = _find_tab()
    if not tab:
        print(f"{get_color('BOLD')}{get_color('RED')}No Miro tab{get_color('RESET')} found.")
        return

    s = CDPSession(tab["webSocketDebuggerUrl"], timeout=10)
    boards = s.evaluate("""
        (function(){
            var items = document.querySelectorAll('[class*="board"], [class*="card"], [data-testid*="board"], a[href*="/board/"]');
            var out = [];
...
def _cmd_explore(tool):
    ...
    if img:
        out_path = Path("/Applications/AITerminalTools/tmp/miro_explore.png")
