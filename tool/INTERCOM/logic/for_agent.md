# INTERCOM Logic — Technical Reference

## Architecture

Standard CDMCP tool pattern:
- `chrome/api.py`: All CDP operations against `app.intercom.com`
- `mcp/main.py`: MCPToolConfig for MCP server registration

## chrome/api.py

Tab discovery: `find_intercom_tab()` searches for `app.intercom.com` via `logic.chrome.session.find_tab()`.

Data source: internal API + DOM scraping fallback.

Public API: `find_intercom_tab()`, `get_auth_state()`, `get_page_info()`, `get_conversations()`, `get_contacts()`.
