...
- **Progressive**: Real-time progress tracking with accurate ETA.
- **Smart**: Local metadata caching for incremental updates.
- **Timezone**: Support for city names, IANA zones, and UTC offsets.
- **Reliable**: Stall detection and automatic retry loop with escalating timeouts.
...
- **Unit Testing**: Standardized testing framework using `unittest`. Run tests in parallel with `TOOL test iCloud`.

## Development Guidelines

1. **Isolation**: Use the `PYTHON` tool dependency for a standalone runtime. Specify dependencies in `tool.json`.
2. **Testing**: Add unit tests in `test/`. Use `TOOL test iCloud` to run them in parallel.
