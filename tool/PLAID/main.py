...
def main():
    ...
    parser = argparse.ArgumentParser(description="Financial data integration via Plaid API", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}PLAID{RESET} tool status:")
        print(f"  MCP package: custom")
        print(f"  Type: custom")
        print(f"  Capabilities: link-accounts, transactions, balances, identity-verification")
        return 0

    if args.command == "config":
        if not args.args or len(args.args) < 2:
            print(f"Usage: PLAID config <key> <value>")
            return 1
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: link-accounts, transactions, balances, identity-verification")
    return 0
