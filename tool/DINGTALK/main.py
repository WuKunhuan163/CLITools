...
def main():
    ...
    parser = argparse.ArgumentParser(description="DingTalk messaging and workspace integration", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}DINGTALK{RESET} tool status:")
        print(f"  MCP package: custom")
        print(f"  Type: custom")
        print(f"  Capabilities: send-message, group-management, webhook")
        return 0
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: send-message, group-management, webhook")
    return 0
