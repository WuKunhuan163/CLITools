"""Test Linear operations via Chrome CDP.

Skips automatically when CDP is unavailable or no Linear tab is open.
"""
import unittest
import sys
...
@unittest.skipUnless(_CDP_OK, SKIP_CDP)
@unittest.skipUnless(_TAB_OK, SKIP_TAB)
class TestLinearUserInfo(unittest.TestCase):
    ...
    def test_get_user_info(self):
        ...
        d = r.get("data", {})
        self.assertTrue(d.get("accountId"), "No accountId")
        self.assertTrue(d.get("email"), "No email")


@unittest.skipUnless(_CDP_OK, SKIP_CDP)
@unittest.skipUnless(_TAB_OK, SKIP_TAB)
class TestLinearPageInfo(unittest.TestCase):
    ...
    def test_get_page_info(self):
        ...
        self.assertIn("url", r)
        self.assertIn("linear", r.get("url", ""))


if __name__ == "__main__":
    if _CDP_OK and _TAB_OK:
        print("[MCP] CDP available, Linear tab found - tests will run.")
    elif _CDP_OK:
        print("[MCP] CDP available but no Linear tab - tests will be SKIPPED.")
    else:
