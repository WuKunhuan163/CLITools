...
MAX_ITERATIONS = 50
_RUN_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "run"
...
class OpenClawCLI:
    ...
    def _setup_llm(self):
        ...
        while step > 0:
            if step == 1:
                ...
                print(fmt_detail(_('get_key_at', 'Get key at: {url}', url=url_map[selected['value']])))
                if existing:
                ...
                if api_key:
                    ...
                    print(fmt_detail(_('using_stored_key', 'Using stored API key.')))
                else:
                    continue
                step = 2
                ...
                if not ready:
                    print(fmt_detail(_('no_providers_ready', 'No providers ready. Configure an API key first.')))
                    step = 1
                    continue
                if len(ready) == 1:
                    self.backend = ready[0]["name"]
    ...
    def _ensure_session(self):
        if not self.session:
            ...
            self._session_line_desc = ""
            sys.stdout.write(
                f"\033[A\033[K{self._stage(done_label, 'done')}\n")
            sys.stdout.flush()
            print(self._detail(
                _("session_started_detail",
    ...
    def _detail(self, text: str, depth: int = 1, styled: bool = False) -> str:
        """Format a detail/sub-info line, aligned under parent stage.

        Delegates to :func:`logic.turing.status.fmt_detail`.
        Default indent is 6 (aligned under stage text after ``> ``).
        """
        indent_n = 8 if depth == 2 else 6
        return fmt_detail(text, indent=indent_n, styled=styled)

    def _thought(self, text: str) -> str:
        """Format an agent thought/reasoning line (dimmed, plain text)."""
        return truncate_to_width(
            f"    {DIM}{text}{RESET}", _get_configured_width())

    def _thought_cont(self, text: str) -> str:
        """Format a continuation line of agent thought (dimmed, plain text)."""
        return truncate_to_width(
            f"    {DIM}{text}{RESET}", _get_configured_width())
    ...
    def _stream_response(self, messages):
        """Stream LLM response, displaying text continuously.

        Content blocks are rendered per type:
        - Plain text / NARRATIVE: normal-weight detail lines
        - THINK block: dimmed detail lines
        - Markers (EXEC, EXPERIENCE, etc.): consumed silently

        The first visible text line replaces the "Waiting..." / "Planning..."
        stage line via cursor-up.

        Returns (response_text, tokens_used) on success, or
        (None, error_message) on failure.
        """
        full_parts = []
        text_buf = ""
        width = _get_configured_width()
        waiting_replaced = False
        tool_calls_received = []
        ...
        if self._tool_schemas:
            stream_kwargs["tools"] = self._tool_schemas

        for chunk in self._provider.stream(messages, **stream_kwargs):
            if not chunk.get("ok"):
                error = chunk.get("error", "Stream error")
                return None, error

            tc = chunk.get("tool_calls")
            if tc:
                tool_calls_received.extend(tc)
                continue

            if chunk.get("done"):
                tokens_used = chunk.get("usage", {}).get("total_tokens", 0)
                if text_buf.strip():
                    if not waiting_replaced:
                        sys.stdout.write(f"\033[A\033[K")
                        waiting_replaced = True
                    self._flush_stream_line(text_buf, width, dimmed=False)
                full_text = chunk.get("full_text", "".join(full_parts))
                if tool_calls_received or chunk.get("tool_calls"):
                    all_tc = tool_calls_received + (
                        chunk.get("tool_calls") or [])
                    return full_text, tokens_used, all_tc
                return full_text, tokens_used, None
            ...
            while "\n" in text_buf:
                ...
                if line.strip() and not line.strip().startswith("TITLE:"):
                    if not waiting_replaced:
                        sys.stdout.write(f"\033[A\033[K")
                        waiting_replaced = True
                    self._flush_stream_line(line, width, dimmed=False)
        ...
        return response_text, 0, tool_calls_received or None

    def _flush_stream_line(self, line: str, width: int,
                           dimmed: bool = True):
        """Print a streamed text line as a wrapped detail line.

        Args:
            dimmed: True for THINK blocks (dimmed), False for NARRATIVE/plain.
        """
        line = line.strip()
        if not line:
            return
        if line.startswith("<<") or line.startswith("TITLE:"):
            return
        formatted = fmt_detail(line, indent=6, wrap=True,
                               styled=not dimmed)
    ...
    def _run_pipeline(self, user_task: str) -> bool:
        ...
        while self._iteration < MAX_ITERATIONS:
            ...
            if self._iteration == 1:
                ...
                print(self._detail(
                    _("task_packaged", "Task packaged. [{ref}]",
                      ref=op_log.ref)))
                print(self._detail(
                    _("sent_to_backend", "Sent to {backend}.",
                      backend=self.backend)))
                waiting_label = _("waiting_for_agent",
                                  "Waiting for agent response...")
                sys.stdout.write(
                    self._stage(waiting_label, "active") + "\n")
                sys.stdout.flush()
            ...
            if response_text is None:
                ...
                if "429" in str(error):
                    sys.stdout.write(f"\033[A\033[K")
                    print(self._stage(
                        _("rate_limited_label", "Rate limited"),
                        "error", desc=_('rate_limited_desc', 'Waiting 60s...')))
            ...
            seg_parsed = parse_structured_tool_calls(
                response_text, tool_calls or [])
            ...
            # Step summary is already displayed during streaming via
            # _stream_response's <<STEP:>> marker detection.

            # Process segments: thoughts (already streamed), then ONE command
            feedback_parts = []
            executed_cmd = None
            for seg in segments:
                if seg["type"] in ("thought", "think", "narrative"):
                    pass  # already displayed during streaming
                elif seg["type"] == "experience":
                    print(self._detail(
                        _("experience_label", "Learned: {exp}", exp=seg['content'])))
                    self.session_mgr.add_message(
                        self.session.id, "system", f"[Experience] {seg['content']}")
                elif seg["type"] == "command" and executed_cmd is None:
                    # DEV BREAKPOINT: after analysis, before command execution
                    if getattr(self, '_dev_breakpoint', False):
                        import json as _bp_json
                        bp_path = _LOG_DIR / "post_analysis_dump.json"
                        _LOG_DIR.mkdir(parents=True, exist_ok=True)
                        ...
                        print(self._detail(
                            _('breakpoint_msg',
                              '[BREAKPOINT] Post-analysis state written to log.')))
                        print(fmt_detail(str(bp_path), indent=4, wrap=True))
                        self._breakpoint_exit = True
                        return True
            ...
            if not feedback_parts:
                if step_complete or not all_commands:
                    feedback_parts.append(
                        "Acknowledged. Continue with <<EXEC: cmd >> "
                        "or <<OPENCLAW_TASK_COMPLETE>>.")
                else:
                    return True

            all_feedback = "\n\n".join(feedback_parts)
            all_feedback += (
                "\n\nContinue. Stream text freely. "
                "Use <<EXEC: cmd >> to run commands. "
                "Use <<OPENCLAW_TASK_COMPLETE>> when done."
            )
            ...
            # "Planning next move..." shown before agent's next response
            planning_label = _("planning_next", "Planning next move...")
            sys.stdout.write(self._stage(planning_label, "active") + "\n")
            sys.stdout.flush()
            time.sleep(0.3)
                ...
                if text.startswith("/"):
                    if text == "/setup":
                        ...
                        if self._provider and self._provider.is_available():
                            ...
                            self._recolor_indicator(display_text, tracker.lines, RED)
                    elif text == "/help":
                        self._mark_done(display_text)
                        self._print_help()
                    elif text == "/new":
