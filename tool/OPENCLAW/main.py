...
def cmd_demo(args):
    """Launch the OpenClaw demo in a browser tab."""
    BOLD = get_color("BOLD")
    GREEN = get_color("GREEN")
    BLUE = get_color("BLUE")
    RESET = get_color("RESET")

    demo_html = Path(__file__).resolve().parent / "logic" / "gui" / "html" / "openclaw_demo.html"
    if not demo_html.exists():
        print(f"  {BOLD}Demo file not found:{RESET} {demo_html}")
        return
    ...
    try:
        ...
        server.start()
        print(f"  {BOLD}{GREEN}Started{RESET} OpenClaw demo at {BLUE}http://127.0.0.1:{port}{RESET}")
        server.open_browser()
        ...
        import webbrowser
        url = f"file://{demo_html}"
        webbrowser.open(url)
        print(f"  {BOLD}{GREEN}Opened{RESET} demo in browser (file mode).")
    except KeyboardInterrupt:
        print(f"\n  {BOLD}Stopped{RESET} demo server.")
...
def main():
    ...
    parser.add_argument("--backend", choices=VALID_BACKENDS,
                        default=None,
                        help="LLM backend (overrides saved config)")
    ...
    subparsers.add_parser("demo", help="Launch the OpenClaw demo in a browser")
    ...
    if args.command == "chat":
        ...
        if len(sys.argv) == 1:
            ...
            print(f"    OPENCLAW demo        Launch interactive demo in browser")
            print()
            print(f"  Management:")
