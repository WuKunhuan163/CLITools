...
class AuthorIdentifier:
    ...
    def identify(self, lines: List[List[Dict[str, Any]]], preference: str = "default") -> Tuple[List[Dict[str, Any]], List[List[Dict[str, Any]]]]:
        ...
        for line in lines:
            ...
            # Author lines are typically at the top, after title, and before Abstract
    ...
    def _is_author_pattern(self, text: str) -> bool:
        text = text.strip(); words = text.split()
        if len(words) < 2 or len(words) > 50: return False
        
        # Patterns: Commas, "and", initials, "By "
        comma_count = text.count(",")
        and_count = text.lower().count(" and ")
        initial_count = len(re.findall(r"\b[A-Z]\.", text))
        
        if text.startswith("By ") or (comma_count >= 2 and initial_count >= 1) or (initial_count >= 3):
            return True
        return False
