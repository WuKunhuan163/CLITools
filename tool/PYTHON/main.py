...
def main():
    ...
    if args.py_update:
        ...
        if update_script.exists():
            ...
            print(f"{RED}" + _("label_error", "Error") + ": " + _("python_update_script_not_found", "Update script not found."))
            sys.exit(1)
    ...
    if len(unknown) > 0 and unknown[0] == "test":
        ...
        if test_dir.exists():
            ...
            print(f"{RED}" + _("label_error", "Error") + ": " + _("python_test_dir_not_found", "Test directory not found."))
            sys.exit(1)
...
def _enable_managed_python():
    ...
    if not default_version:
        print(f"{RED}{BOLD}" + _("label_error", "Error") + ": " + _("python_no_default_configured", "No default version configured."))
        return

    from tool.PYTHON.logic.utils import get_python_exec as get_py_exec
    python_exec = get_py_exec(default_version)
    if not os.path.exists(python_exec) and python_exec != "python3":
        print(f"{RED}{BOLD}" + _("label_error", "Error") + ": " + _("python_default_not_installed", "Default version {version} is not installed.", version=default_version))
        return
    ...
    if success_count > 0:
        ...
        if success_count == 0:
            print(f"{RED}{BOLD}" + _("label_error", "Error") + ": " + _("python_symlinks_failed", "Failed to create symlinks."))
