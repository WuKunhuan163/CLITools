...
def _get_remote_versions():
    ...
    if result.returncode == 0:
        # Extract version names from paths: logic/_/dev/resource/PYTHON/data/install/{version}/PYTHON.json
...
def main():
    ...
    if args.py_update:
        ...
        if update_script.exists():
            ...
            print(f"{RED}{BOLD}" + _("label_error", "Error") + f"{RESET}: " + _("python_update_script_not_found", "Update script not found."))
            sys.exit(1)
    ...
    if len(unknown) > 0 and unknown[0] == "test":
        ...
        if test_dir.exists():
            ...
            for tf in test_files:
                print(f"\n{BOLD}{BLUE}" + _("label_running_test", "Running test") + f":{RESET} {tf.name}")
                # Use current python_exec if possible, otherwise sys.executable
                subprocess.run([sys.executable, str(tf)])
            sys.exit(0)
        else:
            print(f"{RED}{BOLD}" + _("label_error", "Error") + f"{RESET}: " + _("python_test_dir_not_found", "Test directory not found."))
            sys.exit(1)
...
def _enable_managed_python():
    ...
    if not default_version:
        print(f"{RED}{BOLD}" + _("label_error", "Error") + f"{RESET}: " + _("python_no_default_configured", "No default version configured."))
        return

    from tool.PYTHON.logic.utils import get_python_exec as get_py_exec
    python_exec = get_py_exec(default_version)
    if not os.path.exists(python_exec) and python_exec != "python3":
        print(f"{RED}{BOLD}" + _("label_error", "Error") + f"{RESET}: " + _("python_default_not_installed", "Default version {version} is not installed.", version=default_version))
        return
    ...
    # Clean up legacy flat symlinks in bin/
    bin_dir = project_root / "bin"
    for legacy_name in ["python", "python3", "pip", "pip3"]:
    ...
    if success_count > 0:
        ...
        if success_count == 0:
            print(f"{RED}{BOLD}" + _("label_error", "Error") + f"{RESET}: " + _("python_symlinks_failed", "Failed to create symlinks."))
...
def _uninstall_version(version, install_dir=None):
    ...
    if target_dir.exists():
        msg = f"{BLUE}{BOLD}" + _("label_uninstalling", "Uninstalling") + f" {version}..."
        print_erasable(msg)
        ...
        print(f"{RED}{BOLD}{error_label}{RESET}: " + _("python_version_not_installed", "Version {version} is not installed.", version=version))
...
def _install_version(version, install_dir=None, tag_filter=None, platform_filter=None):
    ...
    if not final_version:
        if all_available:
            ...
            print(f"{RED}{BOLD}{error_label}{RESET}: " + _("python_no_versions_found", "No versions found."))
        return False
    ...
    try:
        ...
        if extract_resource(source_zst, target_dir, silent=True):
            ...
            # Clean up checked-out resource files to avoid bloating git
            if full_source_path.exists() and full_source_path.is_relative_to(RESOURCE_ROOT):
                shutil.rmtree(full_source_path, ignore_errors=True)
                # Also unstage from git index
                try:
                    subprocess.run(
                        [_git_bin(), "rm", "-rf", "--cached", source_dir_rel],
