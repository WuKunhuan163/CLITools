...
class TestPythonOps(unittest.TestCase):
    ...
    def test_python_c(self):
        ...
        result = subprocess.run([str(self.python_tool), "--py-version", "3.10.19", "-c", code], capture_output=True, text=True)
    ...
    def test_pip_install(self):
        ...
        result = subprocess.run([str(self.python_tool), "--py-version", "3.10.19", "-m", "pip", "--version"], capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
    ...
    def test_venv_creation(self):
        ...
        result = subprocess.run([str(self.python_tool), "--py-version", "3.10.19", "-m", "venv", str(venv_path)], capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
