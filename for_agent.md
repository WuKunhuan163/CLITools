...
- **`handle_command_line(parser)`**: Standardizes argument processing. Handles `setup`, `install`, `uninstall`, `rule`, `config` commands automatically.
...
## 12. Available Skills
Run `SKILLS show TerminalTools-skills-index` for the full master index of all skills. Run `SKILLS list` to browse all skills.
- `TerminalTools-skills-index`: **Master index** of all skills. Start here to discover available skills and learn how to create new ones.
- `TerminalTools-tool-development-workflow`: Full guide for creating and deploying tools.
- `TerminalTools-turing-machine-development`: Progress display system patterns.
- `TerminalTools-setup-tutorial-creation`: Building interactive setup wizards.
- `TerminalTools-localization`: Multi-language support and translation patterns.
- `TerminalTools-record-cache`: Limit-based record caching with rotation.
- `TerminalTools-code-quality-review`: Auditing for duplicates, dead code, patterns.
- `TerminalTools-session-debug-log`: Debug logging with ToolBase.
- `TerminalTools-tmp-test-script`: Isolated test scripts for debugging.
