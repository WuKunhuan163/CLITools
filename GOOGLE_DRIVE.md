...
- **配置持久化**: 路径配置保存在 `GOOGLE_DRIVE_DATA/sync_config.json` (相对于项目根目录)
- **向后兼容**: 保持与现有代码的完全兼容性
...
```python
# 主要目录
GOOGLE_DRIVE_PROJ/     # 项目代码目录
GOOGLE_DRIVE_DATA/     # 数据和配置目录
...
Connection Check 的行为可通过 `GOOGLE_DRIVE_DATA/template_t_config.json` 配置：
