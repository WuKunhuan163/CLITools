...
class TestExport(unittest.TestCase):
    ...
    def test_help_output(self):
        ...
        result = subprocess.run([
            sys.executable, self.export_py, '--help'
    ...
    def test_run_environment_detection(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_run_environment_detectionEXPORT module not available')
            return
    ...
    def test_json_output_creation(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_json_output_creationEXPORT module not available')
            return
    ...
    def test_variable_name_validation(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_variable_name_validationEXPORT module not available')
            return
    ...
    def test_get_shell_config_files(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_get_shell_config_filesEXPORT module not available')
            return
    ...
    def test_remove_existing_export(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_remove_existing_exportEXPORT module not available')
            return
    ...
    def test_add_export_statement(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_add_export_statementEXPORT module not available')
            return
    ...
    def test_help_function(self):
        ...
        if not EXPORT_AVAILABLE:
            self.skipTest('test_help_functionEXPORT module not available')
            return
    ...
    def test_run_show_compatibility(self):
        ...
        if not run_py.exists():
            self.skipTest('test_run_show_compatibilityRUN.py not found')
            return
    ...
    def test_missing_arguments_error(self):
        ...
        result = subprocess.run([
            sys.executable, self.export_py
        ], capture_output=True, text=True, timeout=10)

        self.assertNotEqual(result.returncode, 0)  # Should fail
        self.assertTrue('Error' in result.stdout or 'Usage' in result.stdout, f"Expected error message, got: {result.stdout}")


    def test_update_option(self):
        """Test --update option functionality"""
        result = subprocess.run([
            sys.executable, self.export_py, '--update'
        ], capture_output=True, text=True, timeout=15)
    ...
    def test_export_variable_dry_run(self):
        ...
        result = subprocess.run([
            sys.executable, self.export_py, 'TEST_VAR_12345', 'test_value_12345'
        ], capture_output=True, text=True, timeout=10)
        ...
        if result.returncode == 0:
            ...
            self.assertTrue('error' in result.stdout.lower() or 'failed' in result.stdout.lower(), 
...
class TestExportIntegration(unittest.TestCase):
    """Integration test cases for EXPORT tool"""

    def setUp(self):
        """Set up integration test environment"""
        pass

    def test_integration_export_only(self):
        """Run EXPORT integration test only"""
        if not INTEGRATION_TEST_AVAILABLE:
            self.skipTest('Integration tests not available - _TEST.py not found')
    ...
    def test_integration_with_google_drive(self):
        """Run EXPORT and GOOGLE_DRIVE integration tests"""
        if not INTEGRATION_TEST_AVAILABLE:
            self.skipTest('Integration tests not available - _TEST.py not found')
...
if __name__ == '__main__':
    ...
    if args.integration_only or args.export_only:
        ...
        suite = unittest.TestSuite()
        suite.addTest(TestExportIntegration('test_integration_export_only'))
        runner = unittest.TextTestRunner(verbosity=2)
        result = runner.run(suite)
        sys.exit(0 if result.wasSuccessful() else 1)
        
    elif args.with_google_drive:
        # Run integration tests with Google Drive
        suite = unittest.TestSuite()
        suite.addTest(TestExportIntegration('test_integration_with_google_drive'))
        runner = unittest.TextTestRunner(verbosity=2)
        result = runner.run(suite)
        sys.exit(0 if result.wasSuccessful() else 1)
