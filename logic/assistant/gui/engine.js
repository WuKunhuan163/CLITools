...
window._toggleToolCall = function(el) {
  el.classList.toggle('expanded');
  if (el.classList.contains('expanded') && el.dataset.lazyDiff) {
    const out = el.querySelector('[data-tc=output]');
    if (out) {
      const diff = renderDiffOutput(el.dataset.lazyDiff, true);
      out.innerHTML = '<div class="diff-view">' + diff.html + '</div>';
    }
    delete el.dataset.lazyDiff;
  }
};
...
class AgentGUIEngine {
  ...
  addSession(id, title, status, mode, doneReason, createdAt) {
    this.sessions[id] = { id, title: title || 'New Task', status: status || 'idle', mode: mode || 'agent', createdAt: createdAt || Date.now() };
    if (doneReason) this.sessions[id].doneReason = doneReason;
  ...
  _renderToolStreamStart(evt) {
    ...
    const el = this._appendAnimated('div', 'tool-call expanded',
      '<div class="tool-call-header" onclick="window._toggleToolCall(this.parentElement)">'
      + headerHtml + '</div>'
  ...
  showPendingModelBanner(model) {
    this._clearPendingModelBanner();
    const isAuto = model === 'auto';
    const div = document.createElement('div');
    div.className = 'model-info model-pending';
    if (isAuto) {
      div.innerHTML =
      ...
      const logo = resolveLogoFn ? resolveLogoFn(model) : (logos[model] || '');
    ...
    this._pendingModelBanner = div;
    this.chatArea.appendChild(div);
    this._scrollEnd();
  }

  _clearPendingModelBanner() {
    if (this._pendingModelBanner) {
      this._pendingModelBanner.remove();
      this._pendingModelBanner = null;
    }
  }

  showModelTimeoutError() {
    this._clearPendingModelBanner();
    const div = document.createElement('div');
    div.className = 'model-info model-error';
    div.innerHTML =
      '<i class="bx bx-x-circle model-info-icon" style="color:var(--red);font-size:16px;"></i>'
      + '<span class="model-name" style="color:var(--red);">Connection timeout</span>';
    this.chatArea.appendChild(div);
    this._scrollEnd();
  }
  ...
  showNetworkError() {
    ...
    div.className = 'model-info model-error';
    div.innerHTML =
      '<i class="bx bx-wifi-off model-info-icon" style="color:var(--red);font-size:16px;"></i>'
      + '<span class="model-name" style="color:var(--red);">Network connection lost</span>';
    this.chatArea.appendChild(div);
    this._scrollEnd();
  }
  ...
  _renderNotice(evt) {
    ...
    if (!text) return;
    const levelIcons = { info: 'bx-info-circle', success: 'bx-check-circle', warning: 'bx-error', error: 'bx-x-circle' };
    const levelColors = { info: 'var(--accent)', success: 'var(--status-ok)', warning: 'var(--orange)', error: 'var(--red)' };
  ...
  _makeToolCall(name, desc, cmd, file) {
    ...
    const el = this._appendAnimated('div', 'tool-call' + (expandable ? '' : ' no-expand'),
      '<div class="tool-call-header" onclick="' + (expandable ? "window._toggleToolCall(this.parentElement)" : '') + '">'
      + headerContent + '</div>'
    ...
    if (output) {
      ...
      if (out) {
        ...
        if (toolType === 'edit') {
          ...
          let addC = 0, remC = 0;
          for (const l of lines) { if (l.startsWith('+')) addC++; else if (l.startsWith('-')) remC++; }
          const stats = el.querySelector('[data-tc=diffstats]');
          if (stats) {
            let parts = [];
            if (addC) parts.push('<span class="added-count">+' + addC + '</span>');
            if (remC) parts.push('<span class="removed-count">-' + remC + '</span>');
