...
from logic._.utils import get_logic_dir, get_rate_color, format_table
...
def audit_lang(lang_code: str, project_root: Path, force: bool = False, turing: bool = False, translation_func: Optional[Callable] = None):
    ...
    if turing:
        ...
        report, report_path = auditor.audit_turing()
        ...
        if total_w > 0:
            ...
            RED = get_color("RED", "\033[31m")
            print(f"{BOLD}{YELLOW}Found {total_w} quality warning(s):{RESET}")
            for w in warnings:
                wtype = w.get("type", "")
                color = RED if wtype == "mixed_language" else YELLOW
                rendered = w.get("rendered", "")
                print(f"  {color}{wtype}{RESET} {DIM}{w.get('file', '')}:{w.get('line', '')}{RESET}")
                print(f"    {w.get('detail', '')}")
                if rendered:
                    print(f"    {DIM}Rendered: {rendered}{RESET}")
        else:
            GREEN = get_color("GREEN", "\033[32m")
            print(f"{GREEN}{BOLD}No quality warnings.{RESET}")
