...
class LangAuditor:
    ...
    def audit_turing(self):
        ...
        In addition to collecting expected output strings for each state,
        the audit now detects:
        - Mixed-language fragments (e.g., CJK status + English code identifier)
        - Code identifiers used as user-facing names (e.g., ``fail_name`` defaulting to ``name``)
        - Bold/color not at the start of the rendered line
        """
        audit_results = {}
        all_warnings = []
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            if file_stages:
                ...
                for lineno, stage_info in file_stages.items():
                    for w in stage_info.get("warnings", []):
                        w["file"] = rel_path
                        w["line"] = lineno
        ...
        report = {
            "stages": audit_results,
            "quality_warnings": all_warnings,
            "summary": {
                "total_warnings": len(all_warnings),
                "mixed_language": sum(1 for w in all_warnings if w["type"] == "mixed_language"),
                "code_identifier": sum(1 for w in all_warnings if w["type"] == "code_identifier_as_name"),
                "bold_not_at_start": sum(1 for w in all_warnings if w["type"] == "bold_not_at_start"),
            },
        }
    ...
    def _extract_stage_info(self, node, file_path):
        ...
        warnings = self._check_output_quality(
            name, active_status, success_status, fail_status,
            active_name, success_name, fail_name, bold_part)
    ...
    @staticmethod
    def _is_code_identifier(text: str) -> bool:
        """Check if text looks like a code identifier (e.g., 'save', 'backup')."""
        if not text:
            return False
        return bool(re.match(r'^[a-z_][a-z0-9_]*$', text))

    def _check_output_quality(self, name, active_status, success_status,
                               fail_status, active_name, success_name,
                               fail_name, bold_part):
        """Check for non-natural-language output and styling issues."""
        warnings = []

        for state_label, status, explicit_name in [
            ("active", active_status, active_name),
            ("success", success_status, success_name),
            ("fail", fail_status, fail_name),
        ]:
            resolved_name = explicit_name if explicit_name is not None else name
            if resolved_name and self._is_code_identifier(resolved_name):
                if status and self._has_cjk(status):
                    warnings.append({
                        "type": "mixed_language",
                        "state": state_label,
                        "detail": f"CJK status '{status}' paired with code identifier "
                                  f"'{resolved_name}' (set {state_label}_name explicitly)",
                        "rendered": f"{status} {resolved_name}",
                    })
                elif status and not self._has_cjk(status) and not re.match(r'^[A-Z]', resolved_name):
                    warnings.append({
                        "type": "code_identifier_as_name",
                        "state": state_label,
                        "detail": f"Name '{resolved_name}' looks like a code identifier, "
                                  f"not user-facing text (set {state_label}_name explicitly)",
                        "rendered": f"{status} {resolved_name}",
                    })

            if bold_part and status:
                full_text = f"{status} {resolved_name}".strip() if resolved_name else status
                if bold_part in full_text and not full_text.startswith(bold_part):
                    warnings.append({
                        "type": "bold_not_at_start",
                        "state": state_label,
                        "detail": f"Bold part '{bold_part}' is not at the start of "
                                  f"'{full_text}' (bold/color should lead the line)",
                    })
