...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kimi AI assistant API integration", add_help=False)
    ...
    if args.command == "status":
        print(f"{BOLD}KIMI{RESET} tool status:")
        print(f"  MCP package: custom")
        print(f"  Type: custom")
        print(f"  Capabilities: chat, long-context, document-analysis")
        return 0

    if args.command == "config":
        if not args.args or len(args.args) < 2:
            print(f"Usage: KIMI config <key> <value>")
            return 1
    ...
    parser.print_help()
    print(f"\n{BOLD}Capabilities{RESET}: chat, long-context, document-analysis")
    return 0
