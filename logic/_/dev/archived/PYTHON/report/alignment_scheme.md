...
To keep the tool branches clean and ensure efficient deployment, we use a separated resource store:
    - **Resource Store**: `logic/_/dev/resource/PYTHON/data/install/`
...
## 4. Maintenance & Updates
    - **Resource Update (`PYTHON update`)**: Synchronizes our remote `logic/_/dev` store with upstream releases.
  ...
  - **Git Migration**: Metadata (`PYTHON.json`) and binary resources are temporarily moved to `logic/_/dev/` for committing to the `tool` branch.
