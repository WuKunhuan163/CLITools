...
GitHub imposes a 10 GB Git LFS storage quota. The `lfs_gc` hook (`hooks/instance/lfs_gc.py`) automatically prunes unreferenced LFS objects after each push. The flow:
1. `auto_save_remote` commits, squashes history, and force-pushes.
...
Large binary files (`*.tar.zst`) are tracked via LFS (`.gitattributes`). The `logic/_/dev/resource/PYTHON/data/install/` directory on the `tool` branch is the primary LFS consumer.
