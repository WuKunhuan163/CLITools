...
Brain logic lives in the SKILLS tool; brain data lives in `data/_/runtime/_/eco/experience/` (a symmetric root directory):
...
`data/_/runtime/` is a **symmetric root directory** — tracked by Git, unlike `data/` which is transient.
...
Lessons are appended to `data/_/runtime/_/eco/experience/lessons.jsonl`.
...
All skills live in `skills/`, brain data in `data/_/runtime/_/eco/experience/`. Both are Git-tracked. Never store institutional knowledge only in `~/.cursor/`. This ensures:
