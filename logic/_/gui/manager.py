...
def handle_gui_remote_command(tool_name: str, project_root: Path, command: str, unknown_args: List[str], translation_helper: Callable) -> int:
    ...
    if found > 0:
        if command == "stop":
            ...
            from logic._.utils.turing.status import fmt_info
...
def run_gui_subprocess(tool_instance, python_exe: str, script_path: str, timeout: int, custom_id: str = None, args: List[str] = None, waiting_label: str = None) -> Dict[str, Any]:
    ...
    from logic._.utils.turing.models.progress import get_global_turing_manager
    manager = get_global_turing_manager()
    gui_slot_id = f"gui_wait_{proc.pid}"
    
    if not is_quiet:
            from logic._.utils.turing.display.manager import truncate_to_width, _get_configured_width
    ...
    try:
        ...
        if not stdout:
            ...
            if is_interrupted:
                ...
                RST = "\033[0m"
                try:
                    from logic._.utils import find_project_root, get_logic_dir
    ...
    filtered_stderr = ""
    if stderr:
        from logic._.gui.tkinter.blueprint.base import filter_tkinter_noise
...
def run_file_fallback(tool_instance, initial_content: str, timeout: int) -> Optional[str]:
    ...
    """
    from logic._.config import get_color
    from logic._.gui.engine import get_sandbox_type
    from logic._.utils import cleanup_old_files
    ...
    try:
        while time.time() < until_ts:
            ...
            if fi > 0 and now - last_focus >= fi:
                from logic._.gui.engine import play_notification_bell
