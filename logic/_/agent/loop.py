...
from logic._.agent.tools import BUILTIN_TOOL_DEFS, ToolHandlers, get_tool_defs_for_mode
from logic._.agent.nudge import (should_nudge, build_nudge_message,
...
class AgentLoop:
    ...
    def __init__(self,
        ...
        if tier >= 2 and project_root and mode in ("agent", "meta-agent"):
            try:
                from logic._.agent.memory import MemoryHandlers, MEMORY_TOOL_DEFS
        ...
        try:
            ...
            while round_num < max_rounds:
                ...
                if (self._tier >= 2 and self._mode == "agent"
                    ...
                    try:
                        from logic._.agent.memory import should_flush_memory
                        ...
                        if should_flush_memory(est_tokens, max_ctx):
                            from logic._.agent.memory import FLUSH_PROMPT_EN
    ...
    def _parse_tool_args(raw: str) -> dict:
        ...
        """
        from logic._.agent._json_repair import repair_and_parse
