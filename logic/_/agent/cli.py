...
#
# Both are imported by logic/_/base/blueprint/base.py for dispatch.
...
def _ensure_workspace(tool_dir: str, project_root: str, tool_name: str = "") -> dict:
    ...
    try:
        ...
        from logic._.workspace.manager import _hash_path
...
def _handle_prompt(args: list, tool_name: str, project_root: str,
    ...
    from logic._.assistant.sandbox import set_tool_sandbox_dir
    ...
    from logic._.agent.loop import AgentLoop
    loop = AgentLoop(
        session=session,
        selected_model=provider,
...
def _execute_tool_via_api(base_url: str, session_id: str,
    ...
    from logic._.assistant.std.registry import ToolContext, STANDARD_TOOLS
    import logic._.assistant.std.tools  # noqa: F401 — ensure tools are registered
...
def _handle_export(args: list, project_root: str):
    ...
    try:
        from logic._.agent.export import export_session
...
def _handle_import(args: list, project_root: str):
    ...
    try:
        from logic._.agent.export import import_session
...
def _handle_brain(args: list, project_root: str):
    ...
    if not args or args[0] == "list":
        from logic._.agent.brain import list_brain_types
        ...
        from logic._.agent.brain import ensure_experience_dir
        d = ensure_experience_dir(project_root, name)
        print(f"  {BOLD}Brain initialized.{RESET} {DIM}{d}{RESET}")
    elif args[0] == "show":
        name = args[1] if len(args) > 1 else "default"
        from logic._.agent.brain import get_experience_dir
...
def _print_dry_run(session: AgentSession, user_text: str,
    ...
    system_prompt = _get_system_prompt(tool_name, mode=mode)
    from logic._.agent.context import build_context
    ...
    from logic._.agent.tools import get_tool_defs_for_mode
