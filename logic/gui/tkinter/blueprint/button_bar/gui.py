...
class ButtonBarWindow(BaseGUIWindow):
    ...
    def setup_ui(self):
        ...
        for i, btn_cfg in enumerate(self.buttons_config):
            ...
            on_click_callback = btn_cfg.get("on_click") # New callback for UI feedback
            
            # Use provided styles or get_button_style() for consistency with USERINPUT
            bg = btn_cfg.get("bg")
            fg = btn_cfg.get("fg")
            ...
            def create_cmd(actual_cmd, should_close, btn_ref_list, callback):
                def wrapper():
                    ...
                    if actual_cmd:
                        actual_cmd()
                    if should_close:
                        self.finalize("success", text)
                return wrapper

            # Build button kwargs dynamically
            # REMOVED: padx/pady as they can break native look on macOS
            btn_kwargs = {
                "text": text,
                "font": font
            }
            ...
            # We'll set the command after creating the button to pass the reference
            btn_ref = []
            btn = tk.Button(button_frame, **btn_kwargs)
            btn_ref.append(btn)
            btn.config(command=create_cmd(cmd, close_on_click, btn_ref, on_click_callback))
            
            # Use pack with fill=tk.X and expand=True for horizontal expansion only
            # This maintains the button's natural height while allowing horizontal stretch
            btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5 if i > 0 else 0, 0))

            # Debug Log styles as requested by user
            try:
                debug_log = Path("/tmp/gui_style_debug.log")
                import time
                with open(debug_log, "a") as f:
                    ts = time.strftime("%H:%M:%S")
                    f.write(f"[{ts}] Button '{text}' - font: {btn.cget('font')}, bg: {btn.cget('bg')}, relief: {btn.cget('relief')}, bd: {btn.cget('bd')}\n")
            except: pass
