...
Follows the symmetrical design pattern: each tool at tool/<NAME>/logic/interface/main.py
...
def get_interface(tool_name: str) -> Optional[ModuleType]:
    ...
    Returns:
        The tool's interface module (tool.<NAME>.logic.interface.main),
        or None if the tool is not installed or has no interface.
    """
    if tool_name in _cache:
        return _cache[tool_name]

    interface_path = _TOOL_DIR / tool_name / "logic" / "interface" / "main.py"
    if not interface_path.exists():
        _cache[tool_name] = None
        return None

    module_name = f"tool.{tool_name}.logic.interface.main"
...
def list_interfaces() -> List[str]:
    ...
    for tool_dir in sorted(_TOOL_DIR.iterdir()):
        if not tool_dir.is_dir():
            continue
        interface_file = tool_dir / "logic" / "interface" / "main.py"
