...
- `logic.lifecycle` (was `logic.tool.lifecycle`) — install/uninstall/list

All imports project-wide now use the canonical `logic.<module>` paths. No backward-compatibility shims remain.

## Gotchas

- **Do not import from `logic.tool.base`** — that file was removed. Import from `logic.tool.blueprint.base`.
