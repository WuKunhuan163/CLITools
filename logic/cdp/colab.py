...
# ---------------------------------------------------------------------------
# Generic Google Drive operations via CDP + gapi.client
# ---------------------------------------------------------------------------
...
def _get_cdp_session(port: int = CDP_PORT) -> Optional["CDPSession"]:
    """Get a CDP session to a Colab tab, or None."""
    tab = find_colab_tab(port)
    if not tab or not tab.get("webSocketDebuggerUrl"):
        return None
    try:
...
def list_drive_files(folder_id: str, query: str = "", port: int = CDP_PORT,
    ...
    Returns: {"success": True, "files": [...]} or {"success": False, "error": "..."}
    """
...
def create_drive_file(name: str, file_type: str, folder_id: str,
    ...
    For native Google types (doc, sheet, etc.), uses metadata-only creation.
    For Colab notebooks, uses multipart upload with notebook JSON.
    For other files, uses multipart upload with content.

    Args:
        name: File name.
        file_type: One of DRIVE_MIME_TYPES keys or a raw MIME type.
        folder_id: Parent folder ID.
        content: Optional file content (for text files, notebooks, etc.)

    Returns: {"success": True, "id": "...", "name": "...", "link": "..."} or error dict.
    """
