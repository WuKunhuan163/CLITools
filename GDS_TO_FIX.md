...
### 根本原因分析
- GDS本身就是bash shell环境
- 使用`bash -c`会创建嵌套的bash进程
- @路径展开可能发生在错误的解析阶段
- 可能需要在`bash -c`的引号前进行路径展开
...
### 需要修复的内容
1. **文档更新**：在GOOGLE_DRIVE.md中明确说明不要使用`bash -c`
2. **错误检测**：当检测到用户使用`bash -c`时，给出警告或自动去除
3. **路径展开优化**：确保@路径和~路径在所有情况下都能正确展开
4. **SIGPIPE处理**：改进管道命令的信号处理，避免显示"Broken pipe"错误

### 优先级
- **高优先级**：问题1（@路径在bash -c中失败）
- **中优先级**：问题2（命令嵌套）、问题4（无输出）
- **低优先级**：问题3（Broken pipe警告，不影响功能）、问题5（显示~字符）
