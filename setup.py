...
def setup_cursor_ide_action(stage=None):
    ...
    project_root = Path(__file__).parent.absolute()
    try:
        from interface.tool import detect_ai_ides, deploy_cursor
        detected = detect_ai_ides(project_root)
        if not detected:
            if stage:
                stage.success_status = "No AI IDE detected"
            return True
        total_deployed = 0
        if "cursor" in detected:
            result = deploy_cursor(project_root)
            total_deployed += len(result.get("deployed", []))
        if stage:
            ide_names = ", ".join(detected)
            stage.success_status = f"Successfully deployed for {ide_names} ({total_deployed} files)"
        return True
    except Exception as e:
        if stage:
            stage.error_brief = str(e)
