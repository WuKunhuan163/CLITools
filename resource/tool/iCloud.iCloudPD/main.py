...
def main():
    ...
    pm.add_stage(TuringStage("auth", auth_action, active_status="Authenticating", active_name="iCloud", success_status="Authenticated", fail_status="Failed to authenticate"))
    ...
    pm = ProgressTuringMachine(project_root=tool.project_root, tool_name="iCloudPD", log_dir=tool.get_log_dir())
    pm.add_stage(TuringStage("scan", scan_action, active_status="Scanning", active_name="iCloud photos/videos", fail_status="Failed to scan"))
    pm.run()
    ...
    pm = ProgressTuringMachine(project_root=tool.project_root, tool_name="iCloudPD", log_dir=tool.get_log_dir())
    pm.add_stage(TuringStage("gathering_objects", gather_action, active_status="Gathering", active_name="photo/video objects", success_status="\r\033[K", success_name=" ", fail_status="Failed to gather"))
    if not pm.run(): sys.exit(1)
