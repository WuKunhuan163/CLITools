...
class PDFExtractor:
    ...
    def extract_pdf_basic(self, pdf_path: Path, page_spec: str = None, output_dir: Path = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 确定要处理的页面
            if page_spec:
                pages = self.parse_page_spec(page_spec, doc.page_count)
            else:
            ...
            for page_num in pages:
                ...
                if image_list:
                    # 使用与basic模式相同的图片合并逻辑
                    merged_images_info = self.merge_nearby_images_to_data(doc, page, image_list, page_num + 1)
    ...
    def extract_pdf_mineru(self, pdf_path: Path, page_spec: str = None, output_dir: Path = None, 
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                output_file = self.handle_mineru_output(pdf_path, output_dir, result.stdout, page_spec)
                print(f"Total processing time: {processing_time:.2f} seconds")
    ...
    def parse_page_spec(self, page_spec: str, total_pages: int) -> List[int]:
        """解析页面规格"""
        pages = []
    ...
    def handle_mineru_output(self, pdf_path: Path, output_dir: Path, stdout: str, page_spec: str = None) -> str:
        ...
        try:
            ...
            if mineru_data_dir.exists():
                ...
                if md_files:
                    ...
                    content = self.fix_image_paths(content, images_dir)
    ...
    def fix_image_paths(self, content: str, images_dir: Path) -> str:
        """修正markdown内容中的图片路径"""
        import re
    ...
    def merge_nearby_images_to_data(self, doc, page, image_list, page_num):
...
class PDFPostProcessor:
            ...
            for img_id, placeholder_type in md_placeholders.items():
                if img_id in json_items:
                    ...
                    new_item = {
                        ...
                        "bbox": [],
                        "processed": False,
                        "processor": self.get_processor_for_type(placeholder_type)
                    }
    ...
    def get_processor_for_type(self, item_type: str) -> str:
        """根据类型获取处理器名称"""
        if item_type == 'image':
            return "Google API"
        elif item_type in ['formula', 'interline_equation']:
        ...
        if arg in ['--help', '-h']:
            ...
            if i + 1 < len(args):
                ...
                if engine_mode not in ['basic', 'basic-asyn', 'mineru', 'mineru-asyn', 'full']:
                    print(f"Error: Invalid engine mode: {engine_mode}")
                    return 1
                i += 2
            else:
            ...
            if i + 1 < len(args) and not args[i + 1].startswith('-'):
                ...
                print("Error: --full requires a PDF file")
                return 1
        elif arg == '--clean-data':
            clean_data = True
            i += 1
            ...
            if i + 1 < len(args):
                ...
                print("Error: --ids requires a value")
                return 1
        elif arg == '--prompt':
            if i + 1 < len(args):
            ...
            if i + 1 < len(args):
                ...
                print("Error: --post-type requires a value")
                return 1
            ...
            if i + 1 < len(args):
                ...
                print("Error: --original-pdf-dir requires a value")
                return 1
        elif arg == '--force':
            post_force = True
            i += 1
            ...
            if i + 1 < len(args):
                try:
                    ...
                    if post_timeout_multi <= 0:
                        print("Error: --unimernet-timeout-multi must be positive")
                        return 1
                    i += 2
                except ValueError:
                    print("Error: --unimernet-timeout-multi must be a number")
                    return 1
            else:
                print("Error: --unimernet-timeout-multi requires a value")
                return 1
        elif arg.startswith('-'):
            ...
            if pdf_file is None:
                ...
                print("Multiple PDF files specified. Only one file is supported.")
                return 1
            i += 1
