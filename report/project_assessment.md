...
A shared `@retry(max_attempts=3, backoff=1.0)` decorator in `logic/utils.py` that any tool can use for flaky API calls. Currently GCS has ad-hoc retry logic; centralizing it would prevent bugs.
