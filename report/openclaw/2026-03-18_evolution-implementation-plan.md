...
```
tool/SKILLS/                        data/_/runtime/_/eco/experience/
...
The `data/_/runtime/` directory is a **symmetric root directory** — each tool (including the project root) has its own `data/_/runtime/` for tracked runtime data. `data/_/runtime/_/eco/experience/` at the project root holds the agent's cross-tool institutional memory.
...
Agent Transcripts (agent-transcripts/*.jsonl)
    ...
    + Lessons (data/_/runtime/_/eco/experience/lessons.jsonl)
        |
        v
...
| Institutional memory | `data/_/runtime/_/eco/experience/lessons.jsonl` |
| Session raw data | `agent-transcripts/*.jsonl` |
| Curated skills | `skills/core/*/SKILL.md` |
| Enforcement | `tool/<NAME>/hooks/pre_commit.py` |
| Improvement suggestions | `data/_/runtime/_/eco/experience/suggestions.jsonl` |
| Evolution audit trail | `data/_/runtime/_/eco/experience/evolution.jsonl` |

### Data vs Logic Separation

Following AITerminalTools convention:
- **`data/_/runtime/_/eco/experience/`**: Runtime data produced by the agent during operation (lessons, suggestions, evolution history). Tracked by Git as a symmetric root directory.
...
### Symmetric Root Directory: `data/_/runtime/`

`data/_/runtime/` is a new symmetric root directory, meaning each tool (including the project root) can have its own `data/_/runtime/` for tracked runtime artifacts. Unlike `data/` (gitignored transient data), `data/_/runtime/` is version-controlled institutional memory. The `experience/` subdirectory holds the evolution system's brain data.
